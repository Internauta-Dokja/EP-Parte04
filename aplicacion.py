def validar_credenciales(usuario, clave):
    with open("usuarios.txt", "r") as file_usuario, open("claves.txt", "r") as file_clave:
        usuarios = file_usuario.read().splitlines()
        claves = file_clave.read().splitlines()
        if usuario in usuarios and clave in claves:
            return True
        else:
            return False
